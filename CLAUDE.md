...
Android 向けの SSH ターミナルクライアント（Kotlin / Jetpack Compose, Material 3 / Material You）。
termux 風の没入型端末を、アドレス帳（ホスト一覧）ホームとセッションドロワーで包む構成。
...
./gradlew :core:test
# 単一テストクラス
./gradlew :core:test --tests "com.uta.terminal.core.ssh.TofuHostKeyVerifierTest"
...
2 モジュール構成。**`:core` は Android 非依存の純 Kotlin(JVM) ライブラリ**（`com.uta.terminal.core`）で、
Android 依存は全て `:app`（`com.uta.terminal`）に閉じ込める。これは Wear 共有とテスト容易性の生命線。
...
### `:app`
- **手動 DI**：`TerminalApp`（Application）→ `AppContainer` が `sessionManager`/`hostKeyStore`/`sessionController`/
...
- `data/` — Room（`TerminalDatabase`/`ProfileEntity`/`ProfileDao`/`ProfileRepository`）で接続プロファイル永続化。
...
- **SSH 利用前に `SshSecurity.ensureBouncyCastle()` を呼ぶ**。Android 同梱の縮小版 "BC" プロバイダを削除し、
  sshj が持ち込む本物の BouncyCastle を最優先登録する（`TerminalApp.onCreate` と `SessionController.connect` で実行）。
